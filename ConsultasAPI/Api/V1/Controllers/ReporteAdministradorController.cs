...
{
    [ApiController]
    [Route("sicoj/consultas/api/v1/reporte-administrador/consultas")]
    public class ReporteAdministradorController : ControllerBase
    {
        private readonly ILogger<OficialPartesController> _logger;
        private readonly IRedisClient _redisClient;
        private readonly IConsultasReporteAdministradorService _consultasReporteAdministradorService;


        public ReporteAdministradorController(ILogger<OficialPartesController> logger, IRedisClient redisClient, IConsultasReporteAdministradorService consultasReporteAdministradorService)
        {
            _logger = logger ?? throw new ArgumentNullException(nameof(logger));
            _redisClient = redisClient ?? throw new ArgumentNullException(nameof(redisClient));
            _consultasReporteAdministradorService = consultasReporteAdministradorService ?? throw new ArgumentNullException(nameof(consultasReporteAdministradorService)); ;
        }
        ...
        public async Task<IActionResult> Exportar_Excel([FromQuery] QueryFilters request)
        {
            try
            {
                ...
                if (filters!.ByIdTipoAsunto.Contains(EnumTipoAsunto.COMERCIO_EXTERIOR.GetHashCode().ToString()) || filters!.ByIdTipoAsunto.Contains(EnumTipoAsunto.IMPUESTOS_INTERNOS.GetHashCode().ToString()))
                {
                    ...
                    var datos = await _consultasReporteAdministradorService.ExportarConsulta(
                ...
                else if (filters!.ByIdTipoAsunto.Contains(EnumTipoAsunto.Cumplimentacion.GetHashCode().ToString()))
                {
                    ...
                    var datos = await _consultasReporteAdministradorService.ExportarCumplimentacion(
                        Filters.GetStringValue(filters!.ByNoAsunto.FirstOrDefault()),
                        Filters.GetStringValue(filters!.ByRfcPromovente.FirstOrDefault()),
                        Filters.GetStringValue(filters!.ByPromovente.FirstOrDefault()),
                        Filters.GetIntValue(filters!.ByIdAdministracion.FirstOrDefault()),
                ...
                { 
                    ...
                    var datos = await _consultasReporteAdministradorService.ExportarReporteGeneral(
                        Filters.GetStringValue(filters!.ByNoAsunto.FirstOrDefault()),
                        Filters.GetStringValue(filters!.ByRfcPromovente.FirstOrDefault()),
                        Filters.GetStringValue(filters!.ByPromovente.FirstOrDefault()),
                        Filters.GetIntValue(filters!.ByIdAdministracion.FirstOrDefault()),
